...
const FIXED_CATEGORIES = ["Cà phê", "Trà sữa", "Trà", "Nước ép", "Latte"];

// Format price to Vietnamese currency (45000 -> 45.000đ)
const formatPrice = (price: number): string => {
  // Handle decimal prices like 45000.00
  const intPrice = Math.round(price);
  return intPrice.toLocaleString("vi-VN") + "đ";
};
...
const buildCategories = (productList: Product[]) => {
  ...
  // Return fixed categories with counts (0 if no products)
  return FIXED_CATEGORIES.map((label) => ({
    label,
